...
public class ChannelIP {
	...
	public TSMessage receive() {
		...
		if (packetInError(ERROR_PROBABILITY, packet))
			packet.setError(); // Set the packet to have an error changing his
								// checksum
	 ...
	 */
	private boolean packetIsLost(double packet_loss_probability,
			TSMessage packet) {
		boolean b = Math.random() < packet_loss_probability;

		// Print trace of the Error
		if (b)
			out.println("* The next packet has been LOST: " + packet);

		return b;
	}

	/**
	 * Modelate randomly if the packet has got an error and print the trace of
	 * the error
	 */
	private boolean packetInError(double packet_error_probability,
			TSMessage packet) {
		boolean b = Math.random() < packet_error_probability;

		// Print trace of the Error
		if (b)
			out.println("* The next packet has an ERROR: " + packet);
