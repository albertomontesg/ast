...
public class TSMessage {
	...
	public TSMessage(int flags, int seqnum, int acknum, byte[] data, int data_length) {
		// Set all the flags
	    if( ( flags & TSMessage.FIN ) > 0 ) fin = true;
	    if( ( flags & TSMessage.SYN ) > 0 ) syn = true;
	    if( ( flags & TSMessage.RST ) > 0 ) rst = true;
	    if( ( flags & TSMessage.PSH ) > 0 ) psh = true;
	    if( ( flags & TSMessage.ACK ) > 0 ) ack = true;
